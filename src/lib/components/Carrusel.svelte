<script lang="ts">
	...
	onMount(async () => {
		const todosLosProductos = await loadProductosCSV();

		// Filtramos y cargamos productos aleatorios
		productos = todosLosProductos.filter(p => p.imagen && p.nombre && p.descripcion_corta).sort(() => 0.5 - Math.random()).slice(0, 3);
